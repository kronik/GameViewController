...
// Number of correct answers before changing title
#define GAME_CORRECT_ANSWERS_LEVEL1 11

// Default cell button size
#define BUTTON_SIZE 250

// Declaration of the game states / modes
typedef enum gameTableMode {
    kModeInit, // Initial state
    kModeStart,// Display game options state
    kModeGame, // Game process state
    kModeScore // Showing score state
} gameTableMode;
...
- (void)showScore {
    ...
    if (self.isTimed) {
        self.task = @[NSLocalizedString(@"Total:", nil),
                      NSLocalizedString(@"Time:", nil),
                      NSLocalizedString(@"Errors:", nil),
                      NSLocalizedString(@"Correct:", nil),
                      NSLocalizedString(@"Correct in sequence:", nil),
                      @"",
                      NSLocalizedString(@"Play again", nil),
...
- (void)viewDidLoad {
    ...
    self.tableView = [[MYTableView alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.height)
...
- (void)showShareResults:(UIView*)button
{
    ...
    if (self.delegate != nil) {
        [self.delegate shareResults: self.totalPassed
                     correctAnswers: self.score
                             errors: self.errors
                  correctInSequence: self.maxInSequence];
    }
}
